...
sudo cat << EOF > /etc/postgresql/17/replica/conf.d/override.conf
listen_addresses = '*'
port = '$replica_port'
EOF

echo "host     all             all             0.0.0.0/0               md5" >> /etc/postgresql/17/replica/pg_hba.conf
echo "host     all             all             ::/0                    md5" >> /etc/postgresql/17/replica/pg_hba.conf
